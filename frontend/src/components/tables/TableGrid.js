...
const TableGrid = ({
  ...
  const getFilteredTables = () => {
    if (!tables) return [];
    // Chỉ lấy bàn thường để lọc
    const regularTables = Object.values(tables).filter(table => table.type !== 'takeaway');
    if (tableFilter === 'available') {
      return regularTables.filter(
  ...
  const takeawayTable = tables['takeaway'];
